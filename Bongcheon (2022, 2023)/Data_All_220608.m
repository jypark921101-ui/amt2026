...
order_current = 'UNYHJSGCDXPKIQOBRAMT';
...
Box_old = Box;
for i = 1:Len
    Box{i} = [Box_old{1}{i}; Box_old{2}{i}];
end

Box_X = retime(Box{19}, (min(Box{19}.Time):seconds(1):max(Box{19}.Time))',"mean");
Box{19} = Box_X;
...
Cal_2.LICOR = movmean(LICOR_2.CO2, SW, 'Endpoints', 'discard');
...
for i = 1:Len
    ...
    V_dry = 1 - (P_ws .* Box_RH/100)/P;
    Cal_2.CO2_dry(:, i) = (Box_Conc./V_dry) * (1013/P);
...
Regression_Consts_3 = zeros(Len, 6);
...
for i = 1:Len
    ...
    AA = Cal_2.LICOR(Search_Window+1-timelag(i):end-Search_Window-timelag(i));
    BB = [Cal_2.T_Raw(:, i), Cal_2.AbH(:,i), Cal_2.CO2_dry(:,i), log(Cal_2.CO2_dry(:,i)), movmean(Cal_2.LICOR(2:end) - Cal_2.LICOR(1:end-1), Search_Window*2, "endpoints", "discard"), ones(length(Cal_2.T_Raw(:, i)), 1)];
    C = BB\AA;
    Regression_Consts_3(i, :) = [C(1), C(2), C(3), C(4), C(5), C(6)];
...
save("220608_CO2_All.mat", "Pressure", "Box", "Cal_1", "Cal_2", "timelag", "Measure_Diff", "Measure", "filter", "Box_Char", "order_current", "order_previous", "Time_Ref", "Smooth_Window_Measure", "Search_Window");
%writetimetable(AAA, "230208_SNUStation.xlsx");

save("220608_CO2_Smoothing_Window.mat", "Cal_2", "LICOR", "Time_Ref");
...
for i = 1:Len
    ...
    RMSE_Timelag(i) = sqrt(mean((Cal_2.LICOR((61:end-60)-timelag(i)) - Cal_2.CO2_dry(:, i)).^2));
    RMSE_Final(i) = sqrt(mean(((Cal_2.LICOR((61:end-60)-timelag(i)) - Cal_2.CO2_dry(:, i)) - (mean(Cal_2.LICOR((61:end-60)-timelag(i)) - Cal_2.CO2_dry(:, i)))).^2));
    RMSE_Cor(i) = sqrt(mean((Cal_2.LICOR((61:end-60)-timelag(i)) - Cal_2.CO2_cor(:, i)).^2));
end
...
hold on
for i = 1:Len
    plot(-Cal_2.LICOR((61:end-60)-timelag(i)) + Cal_2.CO2_cor_2(:, i));
end
%plot(30*(-Cal_2.LICOR(61:end-60) + Cal_2.LICOR(60:end-61)), "color", "black", "linewidth", 1)
